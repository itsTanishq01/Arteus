...
void ShowHierarchyWindow() {
    ...
    ImGui::SameLine();
    if (ImGui::Button(allFoldersExpanded ? "Collapse" : "Expand")) {
    ...
    if (!currentWorkingDirectory.empty()) {
        size_t lastSlash = currentWorkingDirectory.find_last_of(L'\\');
        std::wstring folderName = (lastSlash != std::wstring::npos) ?
            currentWorkingDirectory.substr(lastSlash + 1) :
            currentWorkingDirectory;
        std::string displayName(folderName.begin(), folderName.end());
        ImGui::Text("Current: %s", displayName.c_str());
        ImGui::Separator();
